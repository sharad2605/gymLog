    ...
    <>
      ...
      <div className="hidden md:flex fixed top-16 left-0 h-[calc(100%-64px)] w-64 bg-slate-900 text-white shadow-xl z-40 p-5 flex-col gap-4">
        {links.map((link) => (
          <Link
            key={link.path}
            to={link.path}
            className={`px-3 py-2 rounded-lg hover:bg-yellow-500 transition ${
      ...
      {open && (
        <>
          ...
          />
          <div className="fixed top-16 left-0 h-[calc(100%-64px)] w-64 bg-slate-900 text-white shadow-xl z-40 p-5 flex flex-col gap-4 md:hidden">
            {links.map((link) => (
              <Link
                key={link.path}
                to={link.path}
